...
data Row = Row
  ...
  , _xs :: Vector Double -- replace???
  , _as :: Vector Double -- replace???

  , _receive :: TBQueue (Int, Double)
  , _send    :: TBQueue (Int, Double)
  } 

$(makeLenses ''Row)

initializeRows :: TBQueue (Int, Double) -> TBQueue (Int, Double) -> Matrix Double -> IO [Row]
initializeRows send receive m = do 
...
--       -> Row
-- mkRow v i xs r s = 
--   let last_col = size v - 1
--       aii = v `atIndex` i
--       bi  = v `atIndex` last_col
--       
--       v_coeffs = subVector 0 last_col v
--       
--       as' = dropIndex i v_coeffs
--
--   in Row {
--     aᵢᵢ = aii,
--     bᵢ  = bi,
--     i   = i,
--     
--     -- 3. O 'xs' já veio cortado pelo 'start', então NÃO chama dropIndex de novo!
--     xs = xs, 
--     as = as',
--
--     receive = r,
